#!/usr/bin/env python3
"""
Polymarket Edge Bot v11 — Weekend/Quiet Market Mode
Scans ALL markets with any price data, finds lowest-priced YES orders
to identify genuine mispricing, trades anything with edge > 0.30.
"""
...
def position_size(yes_p, vol, category="Other"):
    ...
    if any(k in (category or "").lower() for k in SKIP_KWDS): return 0
...
# ── Main ────────────────────────────────────────────────────────
cycle  = 0
L("Bot v11 -- Weekend Mode | active scan | mode=%s" % MODE)
...
while True:
    ...
    for m in markets:
        ...
        # Skip clearly resolved archived
        ...
        try:
            yes_p = float(yes_token.get("price", 0))
        except: continue
        
        if yes_p <= 0: 
            # Try live orderbook
            live_p = get_price(yes_token.get("token_id", ""))
            if live_p is None: continue
            yes_p = live_p
        
        if not should_trade(m, yes_p): continue
        
        vol = float(m.get("volume24hr", 0) or 0) or float(m.get("volume", 0) or 0)
        edge = calc_edge(yes_p)
        scored.append((m, yes_p, edge, yes_token.get("token_id", "")))
    
    if not scored:
        L("No opportunities found (%d markets scanned)" % len(markets))
        time.sleep(INTERVAL); continue
    ...
    L("BEST: %s" % q)
    L("  YES=%.4f edge=%.2f | vol=$%s | sz=$%.2f [%s]" % (
        yes_p, edge, ("%d" % int(vol)).replace(",", "_"), sz, cat))
